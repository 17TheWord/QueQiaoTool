...
public class PayloadUtils {
    ...
    /**
     * 解析消息段列表
     * <p>消息可能为字符串、MessageSegment对象，MessageSegment列表</p>
     *
    ...
    public static List<MessageSegment> deserializeMessageSegmentList(JsonElement element, JsonDeserializationContext context) {
        if (element.isJsonArray()) {
            return context.deserialize(element, new TypeToken<List<MessageSegment>>() {
            }.getType());
            ...
            return Collections.singletonList(context.deserialize(element.getAsJsonObject(), MessageSegment.class));
        } else {
