...
public abstract class InfoCommandAbstract extends ServerCommandAbstract {
  ...
  public void execute(Object commandReturner) {
    ...
    Tool.commandReturn(
        commandReturner,
        String.format(
            "当前 Websocket Server 已开启，监听地址为 %s:%d",
            wsServer.getAddress().getHostString(), wsServer.getPort()));
    ...
    Tool.commandReturn(
        commandReturner,
        String.format("当前 Websocket Server 已有 %d 个连接", wsServer.getConnections().size()));
