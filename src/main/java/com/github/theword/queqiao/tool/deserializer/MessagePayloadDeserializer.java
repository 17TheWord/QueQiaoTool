 ...
 *
 * <p>用于将 JSON 数据反序列化为 MessagePayload 对象
 */
public class MessagePayloadDeserializer implements JsonDeserializer<MessagePayload> {
     ...
     * 反序列化 Message 消息
     *
     * <p>接管所有 MessagePayload 及其子类的反序列化任务
     *
     * @param json    Json数据
     * @param typeOfT 目标类型
    ...
    public MessagePayload deserialize(
        ...
        if (jsonObject.has("uuid") || jsonObject.has("nickname")) {
            ...
            privatePayload.setMessage(
                    PayloadUtils.deserializeMessageSegmentList(jsonObject.get("message"), context));
