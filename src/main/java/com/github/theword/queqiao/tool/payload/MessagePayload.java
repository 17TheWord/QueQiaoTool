 ...
 *
 * <p>表示一条消息的负载，包含多个消息段（MessageSegment）。
 *
 * <p>在系统中用于承载需要发送或接收的消息内容，并在需要时转换为纯文本表示。
 */
public class MessagePayload {
    ...
    public String toString() {
        return message.stream().map(MessageSegment::getData).map(CommonTextComponent::getText).collect(Collectors.joining());
    }
}
