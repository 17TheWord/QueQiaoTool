...
public class TestUtils {
   ...
   *
   * <p>The processing service shall be provided by the specific server
   *
   * @param message json message string
   * @param parseJsonToEventImpl parseJsonToEventImpl
   * @return Response response
   */
  public static Response testParseJsonMessage(
      String message, ParseJsonToEventService parseJsonToEventImpl) {
    // 组合消息
    ...
    Response response =
        new Response(200, ResponseEnum.SUCCESS, "success", "No data", basePayload.getEcho());
    try {
      switch (basePayload.getApi()) {
        ...
        case "send_private_msg":
          PrivateMessagePayload privateMessagePayload =
              gson.fromJson(data, PrivateMessagePayload.class);
