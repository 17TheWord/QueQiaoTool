...
public abstract class ReconnectCommandAbstract extends ClientCommandAbstract {
  ...
  public void execute(Object commandReturner, boolean all) {
    String reconnectCount =
        all
            ? CommandConstantMessage.RECONNECT_ALL_CLIENT
            : CommandConstantMessage.RECONNECT_NOT_OPEN_CLIENT;
    ...
    wsClientList.forEach(
        wsClient -> {
          if (all || !wsClient.isOpen()) {
            ...
            Tool.commandReturn(
                commandReturner,
                String.format(CommandConstantMessage.RECONNECT_MESSAGE, wsClient.getURI()));
          } else {
