...
public class TitlePayload {
  ...
  public TitlePayload(
      List<MessageSegment> title,
      List<MessageSegment> subtitle,
      int fadein,
      int stay,
      int fadeout) {
    this.title = title;
  ...
  public String toString() {
    String titleStr =
        "Title:" + title.stream().map(MessageSegment::toString).collect(Collectors.joining(""));
    if (subtitle != null)
      titleStr +=
          "\nSubTitle:"
              + subtitle.stream().map(MessageSegment::toString).collect(Collectors.joining(""));
    return titleStr;
  }
}
